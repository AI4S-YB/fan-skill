...
def match_condition(condition, profile):
    ...
    if condition is None:
        return True  # No condition = always match (stability rule)
    for key, val in condition.items():
        if key not in profile:
            return False
        actual = str(profile[key])
        if isinstance(val, bool):
            # Boolean match: both must be True/truthy or both False/falsy
            profile_bool = str(profile[key]).lower() in ('true', '1', 'yes')
            if profile_bool != val:
                return False
...
for case in cases:
    ...
    if matched is None and expected.get('rule_id') == 'env-fallback-999':

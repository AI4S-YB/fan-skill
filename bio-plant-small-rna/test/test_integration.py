...
def main():
    ...
    catalog_files = [
        "preprocessing.md", "mirna-prediction.md", "target-prediction.md",
        "degradome.md", "differential-mirna.md", "visualization.md"
    ]
    ...
    with open(nb_path, "r") as f:
        nb = f.read()
    for tool in ["miRDeep2", "psRNATarget", "DESeq2", "FastQC", "Cutadapt"]:
        result.assert_true(tool in nb, f"Notebook references {tool}")

    # 5. References file mentions miRBase
    ref_path = os.path.join(SKILL_DIR, "references", "small-rna-plant-special.md")
    with open(ref_path, "r") as f:
        ref = f.read()
    result.assert_true("miRBase" in ref, "References mentions miRBase")

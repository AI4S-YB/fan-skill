...
多倍体植物（小麦、棉花、油菜、马铃薯）的甲基化组分析面临特殊挑战：

**亚基因组甲基化差异**：
- 多倍体中不同亚基因组的甲基化水平通常不同——称为"亚基因组甲基化偏倚"（subgenome methylation bias）
- 例如在六倍体小麦中，B 亚基因组的 CG 甲基化水平通常高于 A 和 D 亚基因组
- 如果已有亚基因组分型组装，分别分析每个亚基因组的甲基化水平
- 如果参考基因组未分型，使用 SNP 标记将 reads 分配到亚基因组（需要亲本物种的 SNP 信息）

**亚基因组间的甲基化互作**：
- 多倍化过程中，一个亚基因组的 TE 通过 RdDM 产生的 siRNA 可以引导另一亚基因组同源位点的甲基化（跨亚基因组表观遗传互作）
- 检测方法：寻找在 A 亚基因组 TE 附近有高 CHH 甲基化，同时在 B 亚基因组同源 TE 位置也有高 CHH 甲基化的位点
- 这在作物改良中很重要——去甲基化剂或 epiRIL 可以释放被表观沉默的有益等位基因
...
**从头分析策略**：
1. 首先计算全基因组的 CG/CHG/CHH 甲基化水平并与已知植物范围比较（CG ~20-60%, CHG ~5-30%, CHH ~1-10%）
2. 使用 sliding window（如 200bp window）而非单碱基来分析甲基化模式（弥补低覆盖度）
3. 识别 putative DMR 后，优先关注基因启动子区域（TSS +/-1Kb）和 TE 区域

**表观等位基因（epiallele）鉴定**：
- 非模式植物（特别是无性繁殖作物：马铃薯、甘薯、甘蔗）中，表观等位基因变异是重要的多样性来源
- 通过比较多个品种/个体的甲基化模式，识别稳定遗传的甲基化差异区域（stably inherited DMR）
- 与 SNP 标记比较，验证 DMR 是否独立于 DNA 序列变异（纯表观遗传变异）

### 组织培养和再生过程中的甲基化变化

植物组织培养和体细胞胚胎发生过程中经常发生 DNA 甲基化重编程：

- 愈伤组织诱导过程中通常出现全基因组低甲基化（特别是 CHH 甲基化）
- 再生植株可能携带"体细胞无性系变异"（somaclonal variation），部分由甲基化改变引起
- 对于转基因和基因编辑作物（CRISPR/Cas9 编辑后经过组织培养），建议对再生植株进行甲基化组分析（至少 targeted bisulfite sequencing），确认未发生不利的表观遗传变异

### 田间 vs 温室环境的甲基化差异

- 田间植物的甲基化水平通常高于温室（特别是 CHH 甲基化，受环境胁迫触发 RdDM）
- 干旱、高温、盐胁迫均可诱导特定 loci 的 CHH 超甲基化
- 这些环境诱导的甲基化变化可能是可遗传的（跨代表观遗传）
- 比较田间和温室样本的甲基化组应注意：差异可能来源于环境效应而非基因型效应

### 作物特异性甲基化数据库

| 数据库 | 覆盖物种 | 内容 |
|--------|---------|------|
| Plant MethylDB | 30+ 植物物种 | 全基因组甲基化谱、DMR、与表达的关联 |
| SoyBase Methylation | 大豆 | 大豆各组织的甲基化组参考 |
| MaizeGDB Epigenome | 玉米 | 玉米各组织/发育阶段的甲基化和组蛋白修饰 |
| RiceENCODE | 水稻 | 水稻多组织的表观基因组数据 |
| Wheat Epigenome | 小麦 | 六倍体小麦亚基因组分辨的甲基化组 |

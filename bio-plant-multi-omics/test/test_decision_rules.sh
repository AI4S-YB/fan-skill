#!/bin/bash
# test_decision_rules.sh — L2: Verify each rule can be triggered
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
SKILL_DIR="${SKILL_DIR:-$(dirname "$SCRIPT_DIR")}"

echo "=== L2: Decision Rule Reachability Test (Multi-Omics) ==="
...
for case in cases:
    ...
    if matched is None and expected.get('rule_id') == 'mo-fallback-999':
